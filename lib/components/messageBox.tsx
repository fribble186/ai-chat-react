...
import { RoleType, MessageBoxProps, MessageProps, AvatarProps } from './messageBoxTypes';
...
export const MessageBoxContainer = styled.div<{ side: RoleType }>`
    ...
    width: 100%;
    align-items: ${props => props.side === 'assistant' ? 'flex-start' : 'flex-end'};
`;
...
export const MessageContainer = styled.div<{ side: RoleType }>`
    ...
    background-color: ${props => props.side === 'assistant' ? 'rgba(231, 231, 231)' : '#e7f8ff'};
    width: fit-content;
    p {
        margin: 0;
        padding: 0;
